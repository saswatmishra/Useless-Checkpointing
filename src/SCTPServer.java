...
public class SCTPServer extends Thread {
	...
	// }
	public void run() {
		int numberOfClient = MainClass.connectedTo.size();
		int i = 0;
		Thread t[] = new Thread[MainClass.connectedTo.size()];
		while ((BasicCheckPoint.sentEnd == false || MainClass.rcvdEnd < (MainClass.connectedTo
				.size())) && i < numberOfClient) {
			System.out.println(BasicCheckPoint.sentEnd + ":"
					+ MainClass.rcvdEnd);
			RecieveMsg rcvmsg;
			...
			try {
				...
				t[i] = new Thread(rcvmsg);
				t[i].setName("Receive Msg Thread");
				t[i].start();
				i++;
		...
		for (int j = 0; i < MainClass.connectedTo.size(); j++) {
			t[j].yield();
			System.out.println("Yielded to server");
		}
	}
}
